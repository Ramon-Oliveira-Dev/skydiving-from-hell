...
/**
 * EvilEye Component — React Bits / WebGL Shader
 * Procedural evil eye shader with animated iris, slit pupil, and fiery outer glow.
 *
 * Adapted for Skydiving From Hell (S.D.F.H.) Hero section.
 */
...
export default function EvilEye({
  ...
  useEffect(() => {
    ...
    function onMouseMove(e: MouseEvent) {
      ...
      const cx = ((e.clientX - rect.left) / (rect.width || 1)) * 2 - 1;
      const cy = -(((e.clientY - rect.top) / (rect.height || 1)) * 2 - 1);
      mouse.tx = Math.max(-1.8, Math.min(1.8, cx));
      mouse.ty = Math.max(-1.8, Math.min(1.8, cy));
    }
    ...
    function onTouchMove(e: TouchEvent) {
      ...
      const cx = ((touch.clientX - rect.left) / (rect.width || 1)) * 2 - 1;
      const cy = -(((touch.clientY - rect.top) / (rect.height || 1)) * 2 - 1);
      mouse.tx = Math.max(-1.8, Math.min(1.8, cx));
      mouse.ty = Math.max(-1.8, Math.min(1.8, cy));
    }
